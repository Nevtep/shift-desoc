...
describe("E2E Tests - Base Sepolia Deployed Contracts", function () {
    ...
    let user1Address: string, user2Address: string, user3Address: string, user4Address: string, user5Address: string;
    ...
    before(async function () {
        ...
        draftsManager = await ethers.getContractAt("DraftsManager", CONTRACT_ADDRESSES.draftsManager);
        governor = await ethers.getContractAt("ShiftGovernor", CONTRACT_ADDRESSES.shiftGovernor);
        countingMultiChoice = await ethers.getContractAt("CountingMultiChoice", CONTRACT_ADDRESSES.countingMultiChoice);
        valuableActionRegistry = await ethers.getContractAt("ValuableActionRegistry", CONTRACT_ADDRESSES.valuableActionRegistry);
        claims = await ethers.getContractAt("Claims", CONTRACT_ADDRESSES.claims);
        verifierPool = await ethers.getContractAt("VerifierPool", CONTRACT_ADDRESSES.verifierPool);
        workerSBT = await ethers.getContractAt("WorkerSBT", CONTRACT_ADDRESSES.workerSBT);
    ...
    async function setupTestTokens() {
        ...
        try {
            ...
            const hasMinterRole = await membershipToken.hasRole(MINTER_ROLE, deployerAddress);
            
            if (hasMinterRole) {
                // Mint tokens to test users
                await membershipToken.connect(deployer).mint(user1Address, tokensPerUser, "E2E test allocation");
                await membershipToken.connect(deployer).mint(user2Address, tokensPerUser, "E2E test allocation");
                await membershipToken.connect(deployer).mint(user3Address, tokensPerUser, "E2E test allocation");
                await membershipToken.connect(deployer).mint(user4Address, tokensPerUser, "E2E test allocation");
                await membershipToken.connect(deployer).mint(user5Address, tokensPerUser, "E2E test allocation");
                ...
                console.log("⚠️ Deployer doesn't have minter role, using existing tokens");
            }
        } catch (error) {
    ...
    describe("Contract Connections", function () {
        it("Should connect to all deployed contracts", async function () {
            ...
            console.log("💰 Total membership token supply:", ethers.formatEther(totalSupply));
    ...
    describe("Governance Workflow", function () {
        ...
        it("Step 1: Should create a community request", async function () {
            ...
            const tx = await requestHub.connect(user1).createRequest(
                COMMUNITY_ID,
            ...
            const event = receipt.logs.find((log: any) => 
                log.topics[0] === ethers.id("RequestCreated(uint256,uint256,address,string)")
            );
        ...
        it("Step 2: Should create draft from request", async function () {
            ...
            const calldatas = [communityRegistry.interface.encodeFunctionData("updateParameter", [COMMUNITY_ID, paramKey, newValue])];
            ...
            const tx = await draftsManager.connect(user2).createDraft(
                requestId,
                actionBundle,
        ...
        it("Step 3: Should collaborate on draft", async function () {
            ...
            // Create new version
            await draftsManager.connect(user3).snapshotVersion(draftId, "ipfs://QmE2ETestDraftV2");
        ...
        it("Step 4: Should escalate draft to proposal", async function () {
            ...
            const proposalEvent = receipt.logs.find((log: any) => {
                ...
                return log.topics && log.topics[0] === ethers.id("ProposalCreated(uint256,address,address[],uint256[],string[],bytes[],uint256,uint256,string)");
            });
        ...
        it("Step 5: Should vote on proposal", async function () {
            ...
            console.log("⏳ Waiting for voting delay:", votingDelay.toString(), "blocks");
        ...
        it("Step 6: Should execute proposal", async function () {
            ...
            const descriptionHash = ethers.keccak256(
                ethers.toUtf8Bytes("Proposal: Update Community Debate Window to 2 days for better deliberation")
            );
    ...
    describe("Work Verification Workflow", function () {
        ...
        it("Step 2: Should setup verifiers", async function () {
            ...
            await membershipToken.connect(user2).approve(CONTRACT_ADDRESSES.verifierPool, bondAmount);
            await verifierPool.connect(user2).registerVerifier(bondAmount);
            
            await membershipToken.connect(user3).approve(CONTRACT_ADDRESSES.verifierPool, bondAmount);
            await verifierPool.connect(user3).registerVerifier(bondAmount);
            
            await membershipToken.connect(user4).approve(CONTRACT_ADDRESSES.verifierPool, bondAmount);
        ...
        it("Step 3: Should submit work claim", async function () {
            ...
            const tx = await claims.connect(user1).submitClaim(
                actionId,
            ...
            const receipt = await tx.wait();
            const event = receipt.logs.find((log: any) => 
                log.topics[0] === ethers.id("ClaimSubmitted(uint256,address,uint256)")
            );
        ...
        it("Step 4: Should verify claim", async function () {
            ...
            await claims.connect(user2).verifyClaimAsVerifier(claimId, true, "Review meets quality standards");
            await claims.connect(user3).verifyClaimAsVerifier(claimId, true, "Comprehensive analysis provided");  
            await claims.connect(user4).verifyClaimAsVerifier(claimId, false, "Missing security considerations");
        ...
        it("Step 5: Should distribute rewards", async function () {
            ...
            console.log("💰 User1 membership token balance:", ethers.formatEther(membershipBalance));
    ...
    describe("Integration Test", function () {
        it("Should run governance to create action, then use action for work verification", async function () {
            ...
            const totalActions = await valuableActionRegistry.getActionCount(COMMUNITY_ID);
