...
Latest tactical delta (2026-08-03, SHI-27):
- Valuable Action admin authority truth in `apps/web` now resolves against the community AccessManager: `useValuableActionDirectWriteAuthority` verifies `canCall(wallet, ValuableActionRegistry, selector)` per operation (create/edit/activate/deactivate) and fails closed to governance routing on any unreadable or unauthorized result, replacing the previous hardcoded optimistic `hasDirectWrite` assumptions.
- Edit follows an honest path: verified wallets execute `ValuableActionRegistry.update(uint256,...)` directly (simulate-first), and unverified wallets get a per-action governance prefill draft plus an `operation=edit` template in the direct proposal builder; the previous silent stubbed edit mutation is gone.
- Activation controls execute real on-chain `deactivate(uint256)` / `activateFromGovernance(uint256,bytes32)` writes when direct authority is verified; otherwise they remain disabled with the governance proposal link as the fail-closed path.
- Admin mutation helpers no longer claim `submitted` without an on-chain transaction: direct-write and governance routes report `pending` payload-validation results until the wallet write or proposal builder actually runs.
- Remaining gap for 3.3 Manager: the indexer per-action authority endpoint (`/communities/:communityId/valuable-actions/:actionId/authority`) still returns fail-closed `authority_not_resolved` and is not consumed by Manager; broader engagement lifecycle truth is tracked separately in `SHI-19`.

Latest tactical delta (2026-08-01, SHI-53):
- Marketplace and housing unavailable/coming-soon states now derive from explicit Manager capability metadata in `apps/web/lib/community-overview/availability.ts` and route keys in `apps/web/lib/community-overview/routes.ts`, instead of static placeholder copy.
- This is a truthfulness/signaling cleanup only: marketplace, housing, and dispute flows remain non-operable until their indexer projections, data reads, and contract-backed Manager actions are implemented.
...
| `3.3 Valuable Action & Engagement` | Partial | Partial | Partial | `contracts/modules/ValuableActionRegistry.sol`, `contracts/modules/Engagements.sol`, `contracts/modules/ValuableActionSBT.sol`, `test/ValuableActionRegistry.t.sol`, `test/Engagements.t.sol`, `test/ValuableActionSBT.t.sol`; engagement handlers in `apps/indexer/src/index.ts`; canonical engagements UI/routes in `apps/web/app/engagements/**` with legacy compatibility wrappers in `apps/web/app/claims/**` and shared components in `apps/web/components/claims/*`; Valuable Action admin slice in `apps/web/app/community/[id]/valuable-actions/page.tsx`, `apps/web/components/valuable-actions/**`, AccessManager-verified authority modes in `apps/web/hooks/useValuableActionAuthorityMode.ts`, and governance prefill templates (create/edit/activate/deactivate) in `apps/web/components/governance/direct-proposal-create.container.tsx`; remaining Manager gap: indexer per-action authority endpoint is fail-closed and unconsumed, and engagement lifecycle truth is tracked in SHI-19 |
...
| `3.9 Marketplace & Commerce` | Implemented | Missing | Missing | `contracts/modules/Marketplace.sol`, `test/MarketplaceHousing.t.sol`, `test/MarketplaceDisputes.t.sol`; no marketplace projection handlers in `apps/indexer/src/index.ts`; marketplace routes now derive explicit unavailable/coming-soon states from capability metadata in `apps/web/lib/community-overview/availability.ts`, `apps/web/lib/community-overview/routes.ts`, `apps/web/app/marketplace/page.tsx`, `apps/web/app/marketplace/offers/page.tsx` |
...
| `3.11 Co-Housing Manager` | Implemented | Missing | Missing | `contracts/modules/HousingManager.sol`, `test/MarketplaceHousing.t.sol`; no housing projection in `apps/indexer/src/index.ts`; housing routes now derive explicit unavailable/coming-soon states from capability metadata in `apps/web/lib/community-overview/availability.ts`, `apps/web/lib/community-overview/routes.ts`, `apps/web/app/housing/page.tsx`, `apps/web/app/housing/reservations/page.tsx` |
...
- Manager app: Functional for requests, drafts, governance proposals, and claims/engagement interactions; marketplace and housing now fail closed from explicit capability metadata with unavailable states, while disputes, token, economic, treasury, and other admin modules still lack operable UX.
...
- Marketplace and housing now fail closed from explicit capability metadata, but community detail and other placeholder surfaces still need the same rigor (`apps/web/app/communities/[communityId]/page.tsx` and remaining placeholder modules).
...
2. Extend capability metadata beyond marketplace and housing placeholder surfaces.
- Reuse explicit capability metadata for remaining placeholder modules so unsupported routes render honest "not available on this deployment" states from config, not static copy.
