...
import { RequestCreateWizardModal } from "../../components/requests/request-create-wizard";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = getI18n(locale).requestsPage;
  return {
    title: t.metaTitle,
...
export default async function RequestsPage({ searchParams }: PageProps) {
  ...
  if (rawCommunityId != null && `${rawCommunityId}`.trim() !== "") {
    const trimmed = `${rawCommunityId}`.trim();
    communityId = /^\d+$/.test(trimmed) ? trimmed : undefined;
  }
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-10 px-6 py-10 sm:py-12">
      <header className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <RequestCreateWizardModal initialCommunityId={communityId ?? undefined} />
        </div>
      </header>
      ...
      <section className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            {communityId
              ? t.listSubFiltered.replace("{id}", communityId.trim())
              : t.listSubAll}
          </p>
        </div>
...
function CommunityFilterStrings({
  currentCommunityId,
  filterHint,
  filterLabel,
  filterPlaceholder,
    ...
    <div className="card-tight flex flex-col gap-4 md:flex-row md:items-center md:justify-between md:gap-6">
      ...
      >
        ...
        <div className="flex flex-shrink-0 flex-row flex-nowrap items-center gap-2">
          ...
          <Link className="btn-ghost inline-flex h-10 shrink-0 items-center justify-center whitespace-nowrap px-3 text-sm" href="/requests">
            {clearLabel}
          </Link>
        </div>
      </form>
