...
export function createDefaultUserSignedStepExecutor(params: {
  ...
  return async (step, session, context) => {
    ...
    if (step === "DEPLOY_STACK") {
      // DEPLOY_STACK remains the only place where static deployment metadata can seed target templates.
      // Subsequent mutable steps must consume run-scoped addresses persisted in session state.
      ...
      const accessManager = (await publicClient.readContract({
      ...
      const deploymentAddresses: DeploymentRunAddresses = {
        ...
        communityToken: modules.communityToken,
        revenueRouter,
        marketplace
      };

      return { txHashes, communityId, deploymentAddresses };
    }
