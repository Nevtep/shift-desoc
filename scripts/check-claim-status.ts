...
async function checkSpecificClaim(claimId: number, claims: any, valuableActionRegistry: any) {
    console.log("\n📋 CLAIM DETAILS:");
    ...
    try {
        ...
        console.log("   ├── Description:", claim.description?.substring(0, 100) + (claim.description?.length > 100 ? "..." : ""));
        ...
        if (claim.updatedAt && claim.updatedAt > 0n) {
            console.log("   └── Updated:", new Date(Number(claim.updatedAt) * 1000).toLocaleString());
        }
        ...
        try {
            ...
            console.log("   ├── Community Reward:", action.communityTokenReward.toString(), "tokens");
            console.log("   ├── Verification Panel:", action.jurorsMin.toString(), "of", action.panelSize.toString(), "required");
            console.log("   ├── Verify Window:", action.verifyWindow.toString(), "seconds");
            console.log("   └── Evidence Types:", "0x" + action.evidenceTypes.toString(16));
        } catch (error) {
        ...
        if (claim.status === 1n) {
            ...
            console.log("   • Each verifier evaluates work quality against standards");
...
async function checkRecentClaims(claims: any, valuableActionRegistry: any, userAddress: string) {
    ...
    try {
        ...
        for (let i = 1; i <= 10; i++) {
            try {
                ...
                const isUserClaim = claim.claimant.toLowerCase() === userAddress.toLowerCase();
                if (isUserClaim) userClaims++;
                
                console.log("📋 Claim #" + i + (isUserClaim ? " (YOUR CLAIM)" : "") + ":");
                console.log("   ├── Claimant:", claim.claimant === userAddress ? "YOU" : claim.claimant);
                console.log("   ├── Action ID:", claim.actionId.toString());
                console.log("   ├── Status:", getStatusName(claim.status));
                console.log("   ├── Created:", new Date(Number(claim.createdAt) * 1000).toLocaleDateString());
                console.log("   └── Evidence:", claim.evidenceCID.substring(0, 30) + "...");
        ...
        if (foundClaims > 0) {
            ...
            console.log("   └── Recent Activity:", foundClaims > 5 ? "High" : foundClaims > 2 ? "Medium" : "Low");
            
            if (userClaims === 0) {
                console.log("\n💡 OPPORTUNITY:");
                console.log("   • No claims submitted yet - consider submitting work for verification");
...
function analyzeClaimStatus(status: bigint, createdAt: bigint) {
    ...
    switch (Number(status)) {
        ...
        case 1: // UNDER_REVIEW
            console.log("   ├── Next Step: Verifiers casting votes on evidence quality");
            console.log("   └── Timeline: Decision within verification window (24-48 hours)");
            
            if (ageHours > 48) {
                console.log("   ⚠️ WARNING: Review taking longer than expected");
                console.log("      May indicate complex verification or verifier availability issues");
            }
            break;
        ...
        case 2: // APPROVED
            ...
            console.log("   └── Timeline: Completed in", ageHours.toFixed(1), "hours");
            ...
            console.log("   • Check rewards: npx hardhat run scripts/check-rewards.ts --network base_sepolia");
            break;
        ...
        case 3: // REJECTED
            ...
            console.log("   • Consider discussing requirements in community channels");
            break;
    ...
    if (Number(status) < 2) { // Still in progress
        console.log("\n⏰ EXPECTED TIMELINE:");
...
main()
    .then(() => {
        ...
        console.log("   • Monitor rewards: npx hardhat run scripts/check-rewards.ts --network base_sepolia");
