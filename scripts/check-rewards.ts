...
async function main() {
  ...
  try {
    ...
    console.log(
      "   └── Purpose: Governance participation, proposal creation, voting",
    );
    ...
    console.log(
      "   └── Purpose: Community treasury participation, service payments",
    );

    // Check ValuableActionSBT balance (achievement tokens)
    sbtBalance = await valuableActionSBT.balanceOf(checkAddress);
    console.log("🏆 Worker SBTs (Achievements):");
    console.log("   └── Balance:", sbtBalance.toString());
    console.log(
      "   └── Purpose: Non-transferable achievement records, reputation",
    );
    ...
    if (sbtBalance > 0n) {
      ...
      try {
        ...
        for (let i = 0; i < Math.min(Number(sbtBalance), 5); i++) {
          try {
            const tokenId = await valuableActionSBT.tokenOfOwnerByIndex(
              checkAddress,
              i,
            );
            ...
            try {
              const tokenURI = await valuableActionSBT.tokenURI(tokenId);
  ...
  try {
    ...
    console.log(
      "👥 Verifier Status:",
      isVerifier ? "REGISTERED ✅" : "NOT REGISTERED",
    );
  ...
  try {
    ...
    console.log(
      "   ├── Governance Power:",
      ethers.formatEther(totalMembership),
      "votes",
    );
    ...
    if (totalSBTs > 0n) {
      ...
      console.log(
        "   ├── Est. Membership Earned:",
        estimatedMembershipFromWork.toString(),
      );
      console.log(
        "   ├── Est. Community Earned:",
        estimatedCommunityFromWork.toString(),
      );
      console.log(
        "   └── Work Value Ratio: " +
          (totalMembership > 0n
            ? (
                (estimatedMembershipFromWork * 100) /
                Number(ethers.formatEther(totalMembership))
              ).toFixed(1) + "%"
            : "N/A"),
      );
    }
  ...
  try {
    ...
    if (!isVerifier && membershipBalance >= ethers.parseEther("100")) {
      console.log(
        "   🎯 Consider registering as verifier (100+ tokens available)",
      );
      console.log(
        "      └── Command: npx hardhat run scripts/register-verifier.ts --network base_sepolia",
      );
    }
    ...
    if (membershipBalance > 0n) {
      ...
      console.log(
        "      └── Monitor: npx hardhat run scripts/monitor-proposal.ts --network base_sepolia",
      );
    }
  } catch (error) {
  ...
  if (hasTokens && hasAchievements) {
    console.log(
      "🏆 Status: ACTIVE CONTRIBUTOR - Earning rewards through verified work",
    );
  } else if (hasTokens) {
    console.log(
      "💰 Status: TOKEN HOLDER - Ready to participate in governance and work",
    );
  } else if (hasAchievements) {
    console.log(
      "🎖️ Status: VERIFIED WORKER - Completed work but minimal token rewards",
    );
  } else {
  ...
  console.log(
    "   • Check proposals: npx hardhat run scripts/monitor-proposal.ts --network base_sepolia",
  );
}
