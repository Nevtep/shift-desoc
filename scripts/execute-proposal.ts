...
async function main() {
    ...
    try {
        ...
        const description = "Create ValuableAction for Development Work Verification Testing";
        ...
        if (proposalState === 4n) {
            ...
            const queueTx = await shiftGovernor.queue(targets, values, calldatas, descriptionHash);
        ...
        const timelockController = await ethers.getContractAt("TimelockController", TIMELOCK_CONTROLLER);
        ...
        const executeTx = await shiftGovernor.execute(targets, values, calldatas, descriptionHash);
        ...
        console.log("   1. Register as verifiers: npx hardhat run scripts/register-verifier.ts --network base_sepolia");
        console.log("   2. Submit work claims: npx hardhat run scripts/submit-claim.ts --network base_sepolia");
        console.log("   3. Verify claims: npx hardhat run scripts/verify-claim.ts --network base_sepolia");
        console.log("   4. Check rewards: npx hardhat run scripts/check-rewards.ts --network base_sepolia");
