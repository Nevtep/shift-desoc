...
async function main() {
    ...
    const verifierPool = await ethers.getContractAt("VerifierPool", VERIFIER_POOL) as any;
    const workerSBT = await ethers.getContractAt("WorkerSBT", WORKER_SBT) as any;
    ...
    try {
        ...
        const isDeployerModerator = await valuableActionRegistry.isModerator(await deployer.getAddress());
        console.log("   Deployer is moderator:", isDeployerModerator);
        
        console.log("\n2. Token Status:");
        const membershipBalance = await membershipToken.balanceOf(await deployer.getAddress());
        console.log("   Membership tokens:", ethers.formatUnits(membershipBalance, 18));
        ...
        console.log("2. Governance system functional (requests, proposals created)");
        console.log("3. Token economics working (minting, delegation, voting power)");
