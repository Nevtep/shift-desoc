...
async function main() {
  ...
  const signerBalance = await membershipToken.balanceOf(
  ...
  try {
    const draftsManager = await ethers.getContractAt(
      "DraftsManager",
      "0xdd90c64f78D82cc6FD60DF756d96EFd6F4395c07",
    );
    ...
    if (communityRequests.length > 0) {
      ...
      console.log(
        "   └── Attempting to create draft for request:",
  ...
  console.log("");
  console.log(
    "🚀 The main Shift DeSoc workflows are validated on Base Sepolia!",
  );
  console.log(
    "📋 Request → Draft → Proposal → Vote → Execute pipeline is ready",
  );
