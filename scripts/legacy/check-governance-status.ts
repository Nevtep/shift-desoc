...
async function main() {
  ...
  try {
    ...
    if (proposalState === 0n) {
      console.log("\n⏳ PROPOSAL IS PENDING");
      console.log(
        "   Waiting for voting to start at block:",
        snapshot.toString(),
      );
      ...
      if (votingPower > 0n) {
        ...
        try {
          const hasVoted = await shiftGovernor.hasVoted(
            PROPOSAL_ID,
            await deployer.getAddress(),
          );
          ...
          if (!hasVoted) {
            ...
            console.log(
              "   New proposal state:",
              stateNames[Number(newState)] || "Unknown",
            );
          }
        } catch (e) {
      ...
      try {
        ...
        console.log(
          "   Note: Would queue proposal here, but need actual calldata from proposal creation",
        );
      } catch (e) {
    ...
    if (lastActionId > 0n) {
      for (let i = 1; i <= Number(lastActionId); i++) {
        ...
        console.log(
          `     Membership reward: ${actionData.membershipTokenReward}`,
        );
        console.log(
          `     Community reward: ${actionData.communityTokenReward}`,
        );
        console.log(
          `     Jurors min/panel: ${actionData.jurorsMin}/${actionData.panelSize}`,
        );
    ...
    if (proposalState === 0n) {
      ...
      console.log(
        "   Start testing work verification with the new ValuableAction",
      );
    }
  } catch (error: any) {
