...
async function main() {
  ...
  try {
    ...
    logSuccess(
      `CommunityRegistry deployed with ${totalCommunities} communities`,
    );
    ...
    const countingMultiChoice = await ethers.getContractAt(
      "CountingMultiChoice",
      ADDRESSES.countingMultiChoice,
    );
    const supportsInterface =
      await countingMultiChoice.supportsInterface("0x01ffc9a7"); // ERC165
  ...
  try {
    ...
    logSuccess(
      `Governor "${governorName}" - Delay: ${votingDelay}, Period: ${votingPeriod}`,
    );
    ...
    if (registryGovernance === ADDRESSES.governor) {
      ...
      logError(
        `ValuableActionRegistry governance mismatch: ${registryGovernance} vs ${ADDRESSES.governor}`,
      );
    }
  } catch (error) {
  ...
  try {
    ...
    const community = await communityRegistry.getCommunity(
      ADDRESSES.communityId,
    );
    logSuccess(
      `Community "${community.name}" registered with ID ${ADDRESSES.communityId}`,
    );

    // Get module addresses
    const modules = await communityRegistry.getCommunityModules(
      ADDRESSES.communityId,
    );
    ...
    const expectedModules = [
      {
        name: "Governor",
        expected: ADDRESSES.governor,
        actual: modules.governor,
      },
      {
        name: "Timelock",
        expected: ADDRESSES.timelock,
        actual: modules.timelock,
      },
      {
        name: "MembershipToken",
        expected: ADDRESSES.membershipToken,
        actual: modules.membershipToken,
      },
      {
        name: "ValuableActionRegistry",
        expected: ADDRESSES.valuableActionRegistry,
        actual: modules.valuableActionRegistry,
      },
    ];
    ...
    for (const module of expectedModules) {
      if (module.expected.toLowerCase() === module.actual.toLowerCase()) {
        ...
        logError(
          `${module.name} module mismatch: expected ${module.expected}, got ${module.actual}`,
        );
      }
    }
  } catch (error) {
  ...
  try {
    ...
    const governorIsExecutor = await timelock.hasRole(
      EXECUTOR_ROLE,
      ADDRESSES.governor,
    );
    ...
    if (governorTimelock.toLowerCase() === ADDRESSES.timelock.toLowerCase()) {
      ...
      logError(
        `Governor timelock mismatch: ${governorTimelock} vs ${ADDRESSES.timelock}`,
      );
    }
  } catch (error) {
  ...
  try {
    ...
    logSuccess(
      `Founder voting power: ${ethers.formatEther(votingPower)} votes`,
    );
    ...
    if (votingPower >= proposalThreshold) {
      logSuccess(
        `Founder can create proposals (has ${ethers.formatEther(votingPower)}, needs ${ethers.formatEther(proposalThreshold)})`,
      );
    } else {
      logError(
        `Founder cannot create proposals (has ${ethers.formatEther(votingPower)}, needs ${ethers.formatEther(proposalThreshold)})`,
      );
    }
  } catch (error) {
  ...
  console.log(
    "\n📊 Deployment Status:",
    allValidationsPassed ? "READY" : "NEEDS_FIX",
  );
