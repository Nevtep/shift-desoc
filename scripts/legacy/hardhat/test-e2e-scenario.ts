...
const CONTRACT_ADDRESSES = {
  ...
  valuableActionRegistry: "0x...", // Update with deployed address
  cohortRegistry: "0x...", // Update with deployed address
  revenueRouter: "0x...", // Update with deployed address
};
...
async function main() {
  ...
  const countingMulti = (await ethers.getContractAt(
    "CountingMultiChoice",
    CONTRACT_ADDRESSES.countingMulti,
  )) as any;
  ...
  const cohortRegistry = (await ethers.getContractAt(
    "CohortRegistry",
    CONTRACT_ADDRESSES.cohortRegistry,
  )) as any;
  const revenueRouter = (await ethers.getContractAt(
    "RevenueRouter",
    CONTRACT_ADDRESSES.revenueRouter,
  )) as any;
  ...
  for (let i = 2; i <= 5; i++) {
    ...
    console.log(
      `💰 Minting ${ethers.formatEther(TOKENS_PER_USER)} tokens to user${i}...`,
    );
    const mintTx = await token
      .connect(deployer)
      .mint(user.address, TOKENS_PER_USER);
    await mintTx.wait();
  ...
  const requestEvent = requestReceipt.logs.find(
    (log: any) => log.fragment?.name === "RequestCreated",
  );
  ...
  const draftEvent = draftReceipt.logs.find(
    (log: any) => log.fragment?.name === "DraftCreated",
  );
  ...
  const comments = [
    {
      user: user3,
      text: "Great idea! This ActionType will help fund development work.",
    },
    {
      user: user4,
      text: "I support this proposal. The parameters look reasonable.",
    },
    {
      user: user5,
      text: "Agreed. The verification requirements are appropriate.",
    },
  ];
  ...
  const finalizeTx = await draftsManager
    .connect(user2)
    .finalizeForProposal(draftId);
  ...
  const proposalEvent = escalateReceipt.logs.find(
    (log: any) => log.fragment?.name === "DraftEscalated",
  );
  ...
  for (let i = 0; i < voters.length; i++) {
    ...
    const voteTx = await governor
      .connect(voter.user)
      .castVoteWithReason(proposalId, voter.vote, voter.reason);
    await voteTx.wait();
  }
  ...
  if (finalState === 4) {
    ...
    console.log(
      `✅ New ActionType created. Total active ActionTypes: ${actionTypes.length}`,
    );
  } else {
