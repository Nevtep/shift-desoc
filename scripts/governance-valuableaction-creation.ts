...
async function main() {
    ...
    const communityRegistry = await ethers.getContractAt("CommunityRegistry", COMMUNITY_REGISTRY);
    const shiftGovernor = await ethers.getContractAt("ShiftGovernor", SHIFT_GOVERNOR);
    const valuableActionRegistry = await ethers.getContractAt("ValuableActionRegistry", VALUABLE_ACTION_REGISTRY);
    const membershipToken = await ethers.getContractAt("MembershipTokenERC20Votes", MEMBERSHIP_TOKEN);
    ...
    try {
        ...
        console.log("   Proposal threshold:", ethers.formatUnits(proposalThreshold, 18));
        ...
        const createActionCalldata = valuableActionRegistry.interface.encodeFunctionData(
        ...
        const description = "Create ValuableAction: Complete Development Task - Testing work verification system with 2-of-3 juror verification, 100 membership token reward, 1 hour cooldown";
        ...
        const proposeTx = await shiftGovernor.propose(targets, values, calldatas, description);
        ...
        const proposalCreatedEvent = proposeReceipt.logs.find(
            (log: any) => log.topics && log.topics[0] === shiftGovernor.interface.getEvent("ProposalCreated")?.topicHash
        );
