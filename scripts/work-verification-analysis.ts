...
async function main() {
    ...
    try {
        ...
        isDeployerModerator = await valuableActionRegistry.isModerator(await deployer.getAddress());
    ...
    try {
        ...
        const isRegistered = await verifierPool.isRegisteredVerifier(await deployer.getAddress());
    ...
    try {
        // Check WorkerSBT
        console.log("\n🏆 WorkerSBT Analysis:");
        const deployerBalance = await workerSBT.balanceOf(await deployer.getAddress());
        console.log("   Deployer SBT balance:", deployerBalance.toString());
        
        // Try to get total supply
        const totalSupply = await workerSBT.totalSupply();
        console.log("   Total SBTs issued:", totalSupply.toString());
        
    } catch (error: any) {
        console.log("   ❌ Error accessing WorkerSBT:", error.message);
    }
    ...
    try {
        ...
        console.log("   Membership token balance:", ethers.formatEther(membershipBalance));
        ...
        console.log("   Community token balance:", ethers.formatEther(communityBalance));
    ...
    try {
        ...
        const isFounder = await valuableActionRegistry.founderWhitelist(deployerAddress, COMMUNITY_ID);
        ...
        if (isDeployerModerator || isFounder) {
            ...
            console.log("   ⚠️ No permissions to create actions (not moderator/founder)");
        }
        ...
        if (error.message.includes("revert")) {
            ...
            console.log("   💡 ValuableActions may require governance approval or founder status");
        }
    }
    ...
    console.log("📋 Ready for work verification workflow once actions are created");
    console.log("");
