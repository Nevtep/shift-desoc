...
const VOTE_REASON =
  "Work meets quality standards with comprehensive documentation and thorough security analysis. Implementation follows best practices and includes proper testing.";
...
async function main() {
  ...
  try {
    ...
    if (updatedClaim.status === 2n) {
      ...
      console.log(
        "      • Once majority (2/3) votes are collected, verification finalizes",
      );
      console.log("      • If approved: automatic reward distribution");
      console.log(
        "      • If rejected: claimant can appeal or improve and resubmit",
      );
    }
    ...
    console.log(
      "   • Check if you are selected for this claim's verification panel",
    );
    console.log(
      "   • Ensure claim is in correct status (Pending or Under Review)",
    );
    ...
    console.log(
      "   • Already voted: Each verifier can only vote once per claim",
    );
    console.log(
