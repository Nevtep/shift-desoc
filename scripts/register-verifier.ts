...
async function main() {
    ...
    const isAlreadyRegistered = await verifierPool.isRegisteredVerifier(signerAddress);
    ...
    if (tokenBalance < BOND_AMOUNT) {
        ...
        console.log("   Deficit:", ethers.formatEther(BOND_AMOUNT - tokenBalance), "tokens");
        console.log("");
    ...
    try {
        ...
        const currentAllowance = await membershipToken.allowance(signerAddress, CONTRACT_ADDRESSES.verifierPool);
        console.log("   Current allowance:", ethers.formatEther(currentAllowance));
        
        if (currentAllowance < BOND_AMOUNT) {
            const approveTx = await membershipToken.approve(CONTRACT_ADDRESSES.verifierPool, BOND_AMOUNT);
        ...
        const isNowRegistered = await verifierPool.isRegisteredVerifier(signerAddress);
        ...
        console.log("   • Use: npx hardhat run scripts/verify-claim.ts --network base_sepolia");
        console.log("");
