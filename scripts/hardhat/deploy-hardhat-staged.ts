...
async function main() {
  if (network.name !== "hardhat") {
    throw new Error("deploy-hardhat-staged.ts must run with --network hardhat");
  }
  ...
  console.log(`✅ Hardhat staged deploy completed (communityId=${communityId})`);
  console.log(`⛽ Total gas used (script scope): ${totalGasUsed.toString()}`);
}

main().catch((error) => {
  console.error("❌ deploy-hardhat-staged failed", error);
  process.exitCode = 1;
});
