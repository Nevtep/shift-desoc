...
async function main() {
  ...
  try {
    ...
    const actionTx = await valuableActionRegistry
      .connect(deployer)
      .createAction(
        COMMUNITY_ID,
    ...
    const reg1Tx = await verifierPool
      .connect(verifier1)
      .registerVerifier(bondAmount);
    await reg1Tx.wait();
    ...
    await membershipToken
      .connect(verifier2)
      .approve(CONTRACT_ADDRESSES.verifierPool, bondAmount);
    const reg2Tx = await verifierPool
      .connect(verifier2)
      .registerVerifier(bondAmount);
    await reg2Tx.wait();
    ...
    await membershipToken
      .connect(verifier3)
      .approve(CONTRACT_ADDRESSES.verifierPool, bondAmount);
    const reg3Tx = await verifierPool
      .connect(verifier3)
      .registerVerifier(bondAmount);
    await reg3Tx.wait();
    ...
    console.log(
      "      ├── Verifier1 registered:",
      await verifierPool.isRegisteredVerifier(await verifier1.getAddress()),
    );
    console.log(
      "      ├── Verifier2 registered:",
      await verifierPool.isRegisteredVerifier(await verifier2.getAddress()),
    );
    console.log(
      "      └── Verifier3 registered:",
      await verifierPool.isRegisteredVerifier(await verifier3.getAddress()),
    );

    // STEP 3: Submit work claim
    console.log("\n📋 STEP 3: Submitting work claim...");

    const claimTx = await claims
      .connect(worker)
      .submitClaim(
        actionId,
    ...
    console.log(
      "   💰 Worker membership token balance:",
    ...
    console.log(
      "   💰 Worker community token balance:",
    ...
    console.log("\n⚖️ Verifier Outcomes:");
    const verifier1Balance = await membershipToken.balanceOf(
      await verifier1.getAddress(),
    );
    const verifier2Balance = await membershipToken.balanceOf(
      await verifier2.getAddress(),
    );
    const verifier3Balance = await membershipToken.balanceOf(
      await verifier3.getAddress(),
    );

    console.log(
      "   ├── Verifier1 balance:",
      ethers.formatEther(verifier1Balance),
    );
    console.log(
      "   ├── Verifier2 balance:",
      ethers.formatEther(verifier2Balance),
    );
    console.log(
      "   └── Verifier3 balance:",
      ethers.formatEther(verifier3Balance),
    );
  } catch (error: any) {
